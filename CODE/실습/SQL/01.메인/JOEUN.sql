...
-- 1.
-- MANAGER_ID 컬럼이 NULL이 아닌 사원을 중복없이 조회
-- 매니저들의 사원 번호
...
-- 2.
-- EMPLOYEE, DEPARTMENT, JOB테이블을 조인하여 조회
...
SELECT E.EMP_ID 사원번호
      ...
      ,CASE
       ...
       END 구분
      ,CASE
            WHEN MOD(SUBSTR(E.EMP_NO, 8, 1), 2) = 0 THEN '여자'
            WHEN MOD(SUBSTR(E.EMP_NO, 8, 1), 2) = 1 THEN '남자'
       END 성별
      -- ,CASE
      --      WHEN MOD(SUBSTR(E.EMP_NO, 8, 1), 2) IN ('2' , '4') THEN '여자'
      --      WHEN MOD(SUBSTR(E.EMP_NO, 8, 1), 2) IN ('1' , '3') THEN '남자'
      -- END 성별
      ,CASE
            WHEN SUBSTR(E.EMP_NO, 8, 1) IN ('1', '2')
            THEN TRUNC (MONTHS_BETWEEN (sysdate, TO_DATE ('19' || SUBSTR (E.EMP_NO, 1, 2), 'YYYY')) /12)
            WHEN SUBSTR(E.EMP_NO, 8, 1) IN ('3', '4')
            THEN TRUNC (MONTHS_BETWEEN (sysdate, TO_DATE ('20' || SUBSTR (E.EMP_NO, 1, 2), 'YYYY')) /12)
       END 나이
      -- ,TRUNC(
      --        MONTHS_BETWEEN(
      --                       SYSDATE, 
